...
public class AircraftSelectMode : IAppMode
{
    ...
    public void OnEnter(ModeContext context)
    {
        ...
        context.Speech.Speak(
            $"Select an Aircraft. {_aircraftList.Count} aircraft available. " +
        ...
        switch (action)
        {
            ...
            case InputAction.Help:
                _context.Speech.Speak(
                    "Aircraft selection. Up and Down arrows to browse aircraft. Enter to select. Escape to go back.",
                    true);
                return ModeResult.Stay;
    ...
    public void OnResume()
    {
        ...
        {
            var aircraft = _aircraftList[_selectedIndex];
            _context.Speech.Speak($"Aircraft selection. {aircraft.Name}, {aircraft.Variant}.", true);
        }
    }
}
